...
var commonErrors = [ ' error ', 'error:', ' errors ', 'failed', 'fatal', 'unable', '[PM2][ERROR]', 'nginx: [emerg]', 'invalid number of arguments' ];
...
var start = function() {
  ...
  if(aliases.length > 0) useAliases = true; 
...
var getNginx = function() {
  ...
  if (settings.nginxConf) {
    ...
    conf = conf.replace(/{{ifAliases}}/gi, (useAliases) ? '' : '#');
